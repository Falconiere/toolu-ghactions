...
|---|---|---|---|
| 🔍 | [**code-review**](./code-review/README.md) | AI pull-request review against an 8-dimension checklist, running one model via OpenRouter (any OpenAI-compatible id) or a native vendor API (DeepSeek, MiniMax, Kimi), on the Vercel AI SDK. Posts a structured verdict and inline suggestions. | — |
...
`code-review` runs a **shape → review → post** pipeline. One model reviews the diff via OpenRouter (default) or a native vendor API (DeepSeek, MiniMax, Kimi); the SDK returns a Zod-validated verdict, which the action anchors to real lines and posts as a single comment.
...
3. **Review** — build the system + user prompt and call the configured model — OpenRouter or a native vendor API (DeepSeek, MiniMax, Kimi) — via the Vercel AI SDK (`generateObject` + a Zod schema). Output is structured by construction with automatic retries; reasoning is disabled on every backend that offers a switch (it is billed against `MAX_TOKENS`, so a thinking model spends the budget before emitting any JSON; where a vendor has no switch, an empty budget-cut response is retried at a doubled budget). A response the schema rejects is recovered where it honestly can be — findings completed before a truncation cut, or a complete response normalized back onto the schema — and only when nothing trustworthy survives does it surface an `error` verdict (never a silent null).
4. **Validate & anchor** — require source-matching evidence for fresh LLM findings, drop hallucinated line numbers and low-confidence findings, dedup by `(path, line, end_line, fingerprint)`, and anchor each finding to a real changed line. Recommendations are derived only from surviving findings.
