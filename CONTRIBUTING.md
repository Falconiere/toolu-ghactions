...
`code-review/` is a TypeScript [node24 JS action](https://docs.github.com/actions). The dev loop runs on [bun](https://bun.sh) (≥ 1.3.14); the shipped bundles (`run/index.cjs` and `sanitize-sarif/index.cjs`) are bundled with esbuild and the tests run on vitest.
...
2. Make a request matching the wire format the action sends (see `src/llm/reviewWithModel.ts` and `src/llm/providers.ts` for the endpoint, headers, and `response_format`).
