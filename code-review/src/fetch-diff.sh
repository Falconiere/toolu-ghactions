...
# Is a path noise (lockfile / minified / sourcemap / build output / generated)?
# Echoes a reason and returns 0 when noise, returns 1 otherwise. Noise files are
# dropped before the diff is built, so generated bytes never reach the model
# prompt: a PR that checks in a built bundle (e.g. web/dist/) was bloating the
# diff to ~3.5MB — 96% minified output — and timing the review out.
noise_reason() {
    local path="$1"
    case "$path" in
        ...
        *.map) echo "sourcemap"; return 0 ;;
        dist/*|*/dist/*|build/*|*/build/*) echo "build-output"; return 0 ;;
    esac
    ...
    # Oversized blob: a file over ~1MB is either generated (data dump, bundle,
    # vendored blob) or would itself blow the prompt budget — drop it. git
    # reports the blob size without reading the content (O(1)), so this also caps
    # the long-line scan below: any file that reaches it is already ≤ 1MB.
    local blob_size
    blob_size=$(git cat-file -s "$REVIEW_HEAD:$path" 2>/dev/null || echo 0)
    if [ "${blob_size:-0}" -gt 1000000 ]; then
        echo "large-file"
        return 0
    fi

    # Minified-by-content: built bundles are often hash-named (index-AbC123.js)
    # and slip past the extension/dir checks, but always carry a pathologically
    # long line. Hand-written source rarely exceeds a few hundred chars; a line
    # over 5000 chars marks the file as machine-generated. awk prints "1" and
    # exits on the FIRST such line — position-independent across the (≤1MB) file.
    # `|| true` swallows the git-show/awk SIGPIPE exit under pipefail.
    local minified
    minified=$(git show "$REVIEW_HEAD:$path" 2>/dev/null \
        | awk '{ if (length($0) > 5000) { print "1"; exit } }' || true)
    if [ "$minified" = "1" ]; then
        echo "minified"
        return 0
    fi
