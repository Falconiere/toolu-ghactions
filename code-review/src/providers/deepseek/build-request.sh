#!/usr/bin/env bash
# providers/deepseek/build-request.sh — OpenAI-compat, response_format: json_object (free-form, not strict).
set -euo pipefail
...
BODY=$(jq -nc \
    --arg model "${INPUT_MODEL:-deepseek-chat}" \
...
# DeepSeek only supports response_format: json_object (free-form), not strict json_schema.
