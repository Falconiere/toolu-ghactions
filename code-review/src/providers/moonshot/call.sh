#!/usr/bin/env bash
# providers/moonshot/call.sh — POST to Moonshot (api.moonshot.ai, OpenAI-compat). Bearer auth, retry, redact.
set -euo pipefail

API_KEY="${API_KEY:-}"
TIMEOUT_SEC="${MOONSHOT_TIMEOUT:-180}"
CONNECT_TIMEOUT="${MOONSHOT_CONNECT_TIMEOUT:-15}"
MAX_ATTEMPTS="${MOONSHOT_MAX_ATTEMPTS:-3}"
    ...
    case "$HTTP_CODE" in
        200|201)
            if ! jq -e . "$TMP_RESPONSE" >/dev/null 2>&1; then
                jq -nc --argjson hc "$HTTP_CODE" '{error:"Moonshot returned non-JSON", http_code:$hc}' >&2; exit 1
            fi
            ...
            if [ -n "$EMBEDDED" ] && [ "$EMBEDDED" != "null" ] \
                ...
                jq -nc --arg e "$EMBEDDED" '{error:"Moonshot returned an embedded error", detail:$e}' >&2; exit 1
            fi
            [ -z "$EMBEDDED" ] || [ "$EMBEDDED" = "null" ] && { cat "$TMP_RESPONSE"; exit 0; }
            ;;
        401|403) jq -nc --argjson hc "$HTTP_CODE" '{error:"Moonshot authentication failed", http_code:$hc}' >&2; exit 1 ;;
        400|402|404|408|422) jq -nc --argjson hc "$HTTP_CODE" --argjson b "$(safe_body)" '{error:"Moonshot request error", http_code:$hc, body:$b}' >&2; exit 1 ;;
        429|5*|000) : ;;
        *) jq -nc --argjson hc "$HTTP_CODE" --argjson b "$(safe_body)" '{error:"Moonshot API error", http_code:$hc, body:$b}' >&2; exit 1 ;;
    esac

    [ "$attempt" -ge "$MAX_ATTEMPTS" ] && { jq -nc --argjson hc "$HTTP_CODE" --argjson b "$(safe_body)" '{error:"Moonshot request failed", http_code:$hc, body:$b}' >&2; exit 1; }
